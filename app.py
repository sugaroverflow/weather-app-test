...
def _is_valid_url(url):
    ...
    try:
        ...
        private_networks = [
            ...
            '172.2', '172.3', '192.168.', 'localhost', '.local'
        ]

        if any(hostname.startswith(prefix) for prefix in private_networks) or hostname.endswith('.local'):
            return False
...
def require_api_key(f):
    ...
    def decorated_function(*args, **kwargs):
        if OPENWEATHER_API_KEY == "your_api_key_here":
            return jsonify({'error': 'No valid API key provided. Please set your API key in the .env file.'}), 401
...
def get_air_quality_level(aqi):
    ...
    return {'level': 'Unknown', 'description': 'Air quality information is not available.'}
...
def process_weather_data():
    ...
    try:
        ...
        if isinstance(transformation, dict):
            ...
            return jsonify({'error': 'Invalid transformation format. Must be a dictionary with operation and parameters'}), 400
...
def get_air_quality():
    ...
    try:
        ...
        if air_quality_response.status_code != 200:
            return jsonify({'error': 'Failed to fetch air quality data'}), air_quality_response.status_code
        ...
        return jsonify({'error': f'Invalid response from weather service: {str(e)}'}), 500
    except Exception as e:
        # Catch-all for other unexpected errors
        return jsonify({'error': f'Server error: {str(e)}'}), 500
